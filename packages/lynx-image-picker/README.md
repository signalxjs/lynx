...
## Gotchas
- **Cancel resolves; failure throws.** The user dismissing the picker resolves `{ cancelled: true, assets: [] }` — that is an answer, not an error. A native failure (the Android Activity Result launcher isn't registered, `launcher.launch` throws, iOS has no view controller to present from, a pick pre-empted by a second `pickImage` while the first is open) rejects with a `SigxError` carrying `code: 'native_error'`. These used to arrive as `{ cancelled: true }`, indistinguishable from a dismiss, so the app silently did nothing. Branch on `cancelled` for the dismiss; use `try`/`catch` for the failure.
