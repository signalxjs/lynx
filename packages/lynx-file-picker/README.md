...
## Gotchas
- **A failed pick throws; a cancelled pick doesn't.** If the picker can't open at all — the Android Activity Result launcher isn't registered (`@sigx/lynx-permissions` missing from the build), the intent can't be launched, or iOS has no view controller to present from — `pick()` rejects with a `SigxError` (`code: 'native_error'`, message `[@sigx/lynx-file-picker] pick failed: …`). Only a genuine dismiss resolves `{ cancelled: true, assets: [] }`; so does a pick superseded by a newer one, or one cut short by Activity teardown. Wrap `pick()` in `try/catch` — an unhandled rejection is fatal on the main thread.
