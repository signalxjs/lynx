...
- **Lynx's background-thread `PerformanceObserver` works only when the framework preserves the native pipeline options.** SignalX does this for the first real element-op flush, so `observe(['pipeline'])` receives `pipeline.loadBundle` and the older `addTimingListener` receives its setup callback. On Lynx 4.0.1, first paint is exposed as the nested `lynxFcp` metric on that pipeline entry rather than as a separate `metric.fcp` callback. This was previously silent because the native load pipeline ended on SignalX's temporary placeholder; see [#982](https://github.com/signalxjs/lynx/issues/982) and [lynx-family/lynx#8405](https://github.com/lynx-family/lynx/issues/8405).
