 ...
 *
 * PKCE's `S256` challenge requires a SHA-256 digest, but the Lynx background JS
 * thread has no guaranteed `crypto.subtle`. This is a frozen, ~80-line
 * implementation of FIPS 180-4 — verified against the standard test vectors in
 * `__tests__/oauth.test.ts`. It never changes, so the maintenance cost is nil.
 */
...
export function sha256(bytes: Uint8Array): Uint8Array {
    ...
    // High 32 bits of the length are 0 for any realistic input.
