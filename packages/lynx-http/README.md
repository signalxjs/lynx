...
> ⚠️ **Don't call a *bare* `fetch(...)` on-device.** The Lynx BG runtime wraps the whole bundle in one `tt.define(…, function(…, fetch, …))` factory, so a bare `fetch` identifier resolves to the engine's factory parameter (a non-WHATWG fetch whose `Response` has no `.headers`) — it shadows the `globalThis.fetch` this package patches. Two reliable bindings: **`import { fetch } from '@sigx/lynx'`** (recommended — explicit and works on every host) or **`globalThis.fetch(...)`** (reads the patched global directly). Both resolve to the sigx implementation on-device. (signalxjs/lynx#373, #378.)
