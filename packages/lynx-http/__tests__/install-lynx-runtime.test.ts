 ...
 * still hand the global to `sigxFetch` — keyed on RUNTIME PRESENCE, not the
 * import-time `isHttpAvailable()` check — because `sigxFetch` resolves the
 * `Http` module lazily at call time.
 *
...
const original = { fetch: globalThis.fetch, Response: globalThis.Response, NativeModules: G['NativeModules'] };
