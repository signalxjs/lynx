...
// site to call `fetch(...)`.
//
// Precedence: on the **Lynx native runtime** the sigx stack REPLACES any
// engine-provided fetch — some Lynx runtimes (0.5.0+) ship a built-in fetch
// whose `Response` lacks WHATWG `headers`/streaming and can't serialize our
// `FormData`, so consumers calling `res.headers.get(...)` would hit
// `undefined`. We key the decision on RUNTIME PRESENCE, not the import-time
// `isHttpAvailable()` check: the `Http` module may not be enumerable on
// `NativeModules` yet when this module is first imported, but `sigxFetch`
// resolves it lazily at call time. Off-Lynx (web, Node/vitest) we only fill
// gaps so the host's real fetch/Headers/etc. stay intact.
{
