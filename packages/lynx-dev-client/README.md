...
- **Uncaught-error visibility** — in dev, hooks the background-thread `lynx.onError` plus `globalThis` `error`/`unhandledrejection` and `console.error`s the message + stack, so uncaught errors show up in the `sigx dev` terminal (not just as the bare native overlay). The iOS error overlay also surfaces the `NSError` `userInfo` detail rather than only `localizedDescription`. (Production error capture/reporting is the opt-in `@sigx/lynx-observability`.)
