...
- **Uncaught-error visibility** — in dev, hooks the background-thread `lynx.onError` plus `globalThis` `error`/`unhandledrejection` and `console.error`s the message + stack, so uncaught errors show up in the `sigx dev` terminal (not just as the bare native overlay). The on-device **error overlay** shows the **reason first** (then the stack), **pages through multiple errors** (`‹ N/M ›`), and has **Copy** / **Copy all** buttons. Android captures Lynx runtime errors via a `LynxViewClient` (parity with iOS's `didReceiveError`). (Production error capture/reporting is the opt-in `@sigx/lynx-observability`.)
