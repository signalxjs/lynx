...
- **Parked gesture registrations** — `gesture-park.ts` (#958) holds a `SET_GESTURE_DETECTOR` whose element isn't bound yet and replays it, in arrival order, when `SET_MT_REF` binds that wvid. Registration normally arrives after the ref op, but not when the detector's owner mounts *earlier* than the element it binds to — a screen that measures itself before rendering its content does exactly that. Dropping the op there was silent and permanent: nothing threw, the batch stayed well-formed, and the gesture was simply dead. A REMOVE arriving while the SET is still parked forgets it, so a component that unmounts inside the window doesn't install a gesture for itself afterwards.
