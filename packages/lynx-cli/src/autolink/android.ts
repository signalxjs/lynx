...
export interface AndroidLinkResult {
    ...
    /** Kotlin source for the startup bundle resolver delegate. */
    bundleResolverCode: string;
    /** FQN of the module-provided startup bundle resolver, if any. */
...
export function linkAndroid(
    ...
    for (const manifest of androidManifests) {
        ...
        // Startup bundle resolver — at most one per platform. Two packages
        // both claiming bundle selection is unresolvable (whose bundle
        // wins?), so fail the prebuild loudly instead of picking one.
        if (android.bundleResolverClass) {
            if (bundleResolverClass) {
                throw new Error(
                    `Two packages declare android.bundleResolverClass: ` +
                    `${bundleResolverPackage} and ${manifest.package}. ` +
                    `Only one startup bundle resolver can be linked — remove one of the packages.`
                );
            }
            bundleResolverClass = android.bundleResolverClass;
    ...
    const bundleResolverCode = generateBundleResolverKotlin(bundleResolverClass);
 ...
 * `GeneratedBundleResolver.resolveStartupBundlePath(context)` before falling
 * back to the baked `main.lynx.bundle` asset. When no linked module declares
 * `android.bundleResolverClass` the body is `return null`, so apps without an
 * OTA package compile and behave exactly as before.
 */
function generateBundleResolverKotlin(resolverClass?: string): string {
    const body = resolverClass
        ? `        return ${resolverClass}.resolveStartupBundlePath(context)`
        : `        return null`;
    return `package com.sigx.lynxgo.modules
 ...
 * Generated by \`sigx prebuild\` — do not edit manually.
 *
 * Consulted by the host before loading the baked \`main.lynx.bundle\` asset.
 * A non-null return is an absolute filesystem path to a bundle the host
 * should load instead (e.g. an OTA update staged by \`@sigx/lynx-updates\`).
 */
object GeneratedBundleResolver {
