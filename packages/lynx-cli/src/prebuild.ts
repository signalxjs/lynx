...
import {
    ...
    androidGeneratedProguardPath, androidDirName, iosDirName,
 ...
 *
 * Always written — even with no rules to contribute — because
 * `app/build.gradle.kts` lists the file unconditionally in `proguardFiles(...)`
 * and Gradle fails the release build if it is missing.
 */
export function writeAndroidProguardRules(cwd: string, config: ResolvedConfig, rules: string[]): void {
    const file = androidGeneratedProguardPath(cwd, config);
    const body = [
        '# Generated by `sigx prebuild` — DO NOT EDIT.',
        '# R8 keep rules contributed by linked native modules via the',
        '# `android.proguardRules` field of their signalx-module.json.',
        '# App-owned rules belong in proguard-rules.pro next to this file.',
        '',
        ...rules,
        '',
    ].join('\n');
    ...
    if (rules.length > 0) {
        log(`Android: wrote ${rules.length} module proguard rule(s)`);
    }
}
...
export async function runPrebuild(opts: PrebuildOptions = {}): Promise<void> {
    ...
    if (buildAndroid && config.platforms.includes('android')) {
        ...
        writeAndroidProguardRules(cwd, config, result.proguardRules);
