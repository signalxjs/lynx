...
- **`fetch` is just there.** Importing `@sigx/lynx` installs a global WHATWG `fetch` (plus `Headers` / `FormData` / `Response` / a `TextDecoder` shim) backed by [`@sigx/lynx-http`](https://github.com/signalxjs/lynx/tree/main/packages/lynx-http) — URLSession / OkHttp underneath, `FormData` multipart uploads that stream picked-file bytes natively, upload progress, and streaming bodies (`res.body.getReader()` for SSE). The native side autolinks automatically; opt out with `excludeModules: ['@sigx/lynx-http']`.
