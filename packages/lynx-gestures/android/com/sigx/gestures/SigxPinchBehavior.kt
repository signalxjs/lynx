...
/**
 * Registers the `<sigx-pinch>` JSX tag with Lynx's UI registry.
 *
 ...
 * `builder.addBehavior(SigxPinchBehavior())` for every `LynxViewBuilder`.
 */
class SigxPinchBehavior : Behavior("sigx-pinch") {
    override fun createUI(context: LynxContext): LynxUI<*> {
        return SigxPinchUI(context)
    }
}
