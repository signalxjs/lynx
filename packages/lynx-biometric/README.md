...
1. **Wrong activity class.** `BiometricPrompt` requires a `FragmentActivity`. The module reads the host from `@sigx/lynx-core`'s shared `SigxActivityHolder`, which captures any `Activity` — but if yours isn't a `FragmentActivity`, the prompt has nothing to attach to and `authenticate` returns `errorCode: 'noActivity'`. Projects scaffolded by `sigx prebuild` already extend `FragmentActivity`; if you customised `MainActivity`, keep it on `FragmentActivity` (or `AppCompatActivity`, which inherits from it).
