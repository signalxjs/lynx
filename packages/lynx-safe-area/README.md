...
| `useSafeAreaFrame(w, h)` | `→ Computed<{ x, y, width, height }>` | The visible content rect in dp/pt, for absolute overlays and modal bounds. Width/height are derived from the viewport size you pass (the keyboard inset is subtracted from the height). |
