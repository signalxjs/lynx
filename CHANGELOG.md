...
- **`@sigx/lynx-list`: chat first paint no longer lands short of the newest message on Android** (#930). With a thread longer than the viewport, opening a chat left the newest one or two messages behind the composer. Root cause was not the pin but the platform node: the resolved `<list>` tag name also selects the engine's *C++* list implementation, and only `list-container` reaches the modern decoupled list — so `bottomInset`'s `custom-list-name="sigx-list"` (#844) had been silently swapping every inset-bearing list onto the legacy `UIList` recycler, whose instant bottom-aligned `scrollToPosition` computes `offset + rv.height − dipToPx(itemHeight ?: 0)` and accounts for neither the real cell height nor the padding carrying the inset. Its *smooth* scroller is correct, which is why sending a message appeared to fix it, and why a three-message demo seed hid the whole thing: with content shorter than the viewport there is nothing to scroll. `SigxListUI` now extends `UIListContainer` and registers under the engine's own `list-container` name, so Android has one list again and no per-element opt-in; `custom-list-name` stays iOS-only, where it maps onto `LynxUICollection` — the class `<list>` already uses there, which is why iOS was never affected.

- **`@sigx/lynx-list`: `bottomInset` actually applies on Android** (#930). It never had. A platform-side inset cannot lift the newest cell above an occluder on the modern list — the C++ scroller clamps `scrollToPosition` against its own content size, so the upward correction is clamped away while a downward one applies (device-proven by biasing the offset both ways). The clearance now *is* content: a trailing spacer cell, which as the last cell makes the ordinary bottom-aligned pin land the newest message exactly `inset` above the viewport bottom — correct too when that message is taller than the screen, since no cell-height arithmetic is involved. Height changes are layout, so the spacer commits on settle (capped at 400 ms) and the motion between settles is a main-thread `translateY`: no relayout, no `layoutcomplete`, no page events, so it still tracks a dragged sheet frame-by-frame. The settled height and the main thread's copy are written in one op batch, so the transform releases on the same frame the spacer grows.
