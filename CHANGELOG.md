...
- **BREAKING — `@sigx/lynx-clipboard`: `setString` is now `Promise<void>` and rejects on failure** (#872). It was synchronous and returned `void`, so a failed write was **silently invisible**. That wasn't theoretical: Android's `setPrimaryClip` genuinely throws for a clip over the binder transaction limit, and the native method returned nothing, so JS could never learn about it. Both native sides gained a callback; the JS unwraps it through core's `unwrapNativeVoid`, rejecting with a `SigxError` (`code: 'native_error'`). Existing `Clipboard.setString(x)` calls keep working — the promise is simply unawaited — but failures stay invisible until callers add `await`, so it's worth a sweep. On web the same call now rejects when the browser denies clipboard-write permission, instead of logging and swallowing it.
