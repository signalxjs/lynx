...
- `@sigx/lynx-emoji` — tapping a category tab now highlights it immediately: switches are two-phase (the highlight paints in its own cheap flush; the grid swap follows one tick later), and visited categories keep their grid mounted behind `use:show`, so revisiting a tab rebuilds zero cells (#602).
