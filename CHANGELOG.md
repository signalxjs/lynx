...
- **`@sigx/lynx-list`: `bottomInset` — frame-synced bottom content inset (Android)** (#844). New prop `bottomInset?: number | SharedValue<number>`: the native recycler keeps that much of its bottom viewport clear, applied directly (no CSS write, no layout pass) so a main-thread SharedValue — `useKeyboardLiftSV()`, a `BottomSheet`'s `onReveal` — can drive it per frame. The package gains its first native side: `SigxListUI extends UIList` under the opt-in platform tag `sigx-list` (via `custom-list-name`, set only when the prop is present), with a `setBottomInset` UI method that composes with CSS padding across relayouts and, in chat mode, keeps the newest item anchored above the inset via same-frame scroll compensation. Chat release detection becomes inset-aware (tracked on the main thread as `scrollTop + inset`) so compensation scrolls can't falsely release the pin or fire spurious `endReached`. Android ships here; iOS follows. Run `sigx prebuild` after upgrading; hosts without the native module (web) ignore the inset.
